...
- 用户级消息防抖：可配置哪些唤醒消息参与防抖，支持 `normal`、`command`、`at`、`reply`。监听窗口内同一用户再次发言时，如果上一条还没开始输出，就会取消上一条并将消息合并为一次请求。窗口时长、参与类型和最多合并条数都可在插件配置中调整。  
- Bot 消息缓存：始终保留最近 5 条 Bot 发言，用于相关性计算与复读屏蔽。  

以上能力已内嵌，其中消息防抖可通过配置项调节窗口时长、参与类型和最多合并条数。
